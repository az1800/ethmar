      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
          {companies.map((company) => (
            <div
              key={company.id}
              className="relative border border-gray-200 p-3 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300"
            >
              <Card
                name={company.Company_name}
                imageLink={company.Company_Logo || ethmarlogoP}
              ...
              {/* Centered edit/delete buttons for dashboard view */}
              <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex justify-between w-full max-w-[300px]  rounded-lg px-3 pt-0.5 ">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
