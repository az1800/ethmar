...
const HeroTextManagement: React.FC = () => {
  ...
  const handleReset = useCallback(
    async (section: HeroSection): Promise<void> => {
      if (
        window.confirm(
          "هل أنت متأكد من إعادة تعيين النصوص إلى القيم الافتراضية؟"
        )
      ) {
        try {
          await fetchHeroSectionsData(true);
        } catch (error) {
          showNotification({
    ...
    [fetchHeroSectionsData, showNotification]
  );

  const getSectionIcon = (pageTitle: string): React.ReactNode => {
    switch (pageTitle) {
  ...
  const getSectionColor = (pageTitle: string): string => {
    switch (pageTitle) {
      case "الصفحة الرئيسية":
